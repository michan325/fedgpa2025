...
class LocalUpdate_FedGPA(object):
    ...
    def update_base_model(self, global_weight):
        # ver.: only update feature extractor
    ...
    def statistics_extraction(self):
        ...
        # py: samples belong to this category / all samples
        py = self.probs_label
        py2 = py.mul(py)
        v = 0
        # h_ref: 存储每个类别的加权特征均值 h_ref.shape --> 10 category x 128 hidden layer size
        h_ref = torch.zeros((self.num_classes, d), device=self.device)

        # add start
        feature_matrix = torch.zeros((self.num_classes, d), device=self.device)
        # add end
        ...
        return v, h_ref, feature_matrix, feature_dict

    def local_training(self, local_epoch, round=0, count_list=[]):
        ...
        # before local training get acc0
