...
def average_weights_weighted_ours(w, avg_weight):
    """
    Returns the average of the weights.
    """
    w_avg = copy.deepcopy(w[0])
    agg_w = torch.tensor(avg_weight)
    for key in w_avg.keys():
        w_avg[key] = torch.zeros_like(w_avg[key]).to(torch.double)
...
def get_head_agg_weight(num_users, Vars, Hs, weights_extractor, *args, **kwargs):
